...
def generation_metrics_fn(list_hyp, list_label):
    ...
    list_hyp_bleu = list_hyp
    list_label_bleu = [[label] for label in list_label]
    list_label_sacrebleu = [[label] for label in list_label]
    
    metrics = {}
    metrics["BLEU"] = bleu._compute(list_hyp_bleu, list_label_bleu, tokenizer=word_tokenize)['bleu'] * 100
